            ...
            <nav class="sidebar-nav">
                <ul id="sidebarnav" class="mb-0">
                    @foreach ($data_sidebar_menu as $sidebar_menu)
                        <li class="sidebar-item {{ $sidebar_menu->id == 6 ? 'setMasterDataList_menu' : '' }}"
                            wire:key='{{ $sidebar_menu->id }}'>
                            <a class="sidebar-link has-arrow primary-hover-bg {{ $sidebar_menu->id == 6 ? 'setMasterDataList' : '' }}"
                            ...
                            <ul aria-expanded="false"
                                class="collapse first-level {{ $sidebar_menu->id == 5 ? 'setMasterDataList_submenu' : '' }}">
                                @foreach ($data_sidebar_submenu as $sidebar_submenu)
                                    @if ($sidebar_menu->id == $sidebar_submenu->f_menu)
                                        <li class="sidebar-item {{ $sidebar_submenu->id == 10 ? 'setMasterDataList' : '' }}"
                                            wire:key='{{ $sidebar_submenu->id }}'>
                                            <a wire:navigate id="sidebar_submenu" href="/{{ $sidebar_submenu->url }}"
                                                class="sidebar-link {{ $sidebar_submenu->id == 10 ? 'setMasterDataList' : '' }}"
                                                data-id="{{ $sidebar_submenu->id }}">
