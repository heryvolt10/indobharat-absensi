...
new class extends Component
{
    ...
    public $id_header, $nama, $ket, $seq = '0', $icon, $url, $f_menu, $f_menu_tag, $f_status, $f_status_tag, $f_org, $f_org_tag;

    public $filterSearch = '', $filterStatus = '2', $listCount = '0';
    public $sortField = 'menu_seq', $sortDir = 'ASC';
    ...
    public function store($id = "")
    {
        ...
        $this->id_header = $id;
        if ($this->id_header != "") {
            $data_render = M_users_submenu::detail($this->id_header);
            $this->id_header = $data_render->id;
            $this->nama = $data_render->nama;
            $this->seq = $data_render->seq;
            $this->ket = $data_render->ket;
            $this->icon = $data_render->icon;
            $this->url = $data_render->url;
            $this->f_menu = $data_render->f_menu;
            $this->f_menu_tag = $data_render->menu;
    ...
    public function save()
    {
        // ==================================== VALID INPUT =======================================
        $this->valid_role = [
            'nama' => 'required|max:250|unique:users_submenu,nama,' . $this->id_header,
            'ket' =>  'max:500',
            'seq' => 'required|integer|max:100',
            'f_menu' => 'required',
        ...
        if ($validator->errors()->count() > 0) {
            ...
            try {
                ...
                if ($this->id_header == "") {
                    ...
                    $execDB = M_users_submenu::create($data_output);
                    ...
                    $execDB = M_users_submenu::withTrashed()->findOrFail($this->id_header)->update($data_output);
    ...
    {
        $postDelete = M_users_submenu::withTrashed()->findOrFail($id);
        $postDelete->delete();
