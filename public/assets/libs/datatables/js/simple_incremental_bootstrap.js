...
$.fn.dataTableExt.oPagination.simple_incremental_bootstrap = {
  fnInit: function (oSettings, nPaging, fnCallbackDraw) {
    ...
    $(nPrevious).append(
      $("<span>" + oSettings.oLanguage.oPaginate.sPrevious + "</span>")
    );
    $(nFirst).append($("<span>1</span>"));
    $(nNext).append(
      $("<span>" + oSettings.oLanguage.oPaginate.sNext + "</span>")
    );
    ...
    $(nNext).click(function () {
      if (oSettings.aiDisplay.length < oSettings._iDisplayLength) {
        oSettings._iRecordsTotal =
          oSettings._iDisplayStart + oSettings.aiDisplay.length;
      } else {
        oSettings._iRecordsTotal =
          oSettings._iDisplayStart + oSettings._iDisplayLength + 1;
      }
  ...
  fnUpdate: function (oSettings, fnCallbackDraw) {
    ...
    for (var i = 0, iLen = an.length; i < iLen; i++) {
      ...
      var page =
        Math.round(oSettings._iDisplayStart / oSettings._iDisplayLength) + 1;
      if (page == buttons.length - 1 && oSettings.aiDisplay.length > 0) {
        $new = $(
          '<li class="dynamic_page_item active"><span>' + page + "</span></li>"
        );
      ...
      } else $(buttons[page]).addClass("active");
